...
static ngx_command_t  ngx_stream_ssl_commands[] = {
    ...
    { ngx_string("ssl_spiffe"),		
      ...
      ngx_conf_set_flag_slot,		
      NGX_STREAM_SRV_CONF_OFFSET,		
      offsetof(ngx_stream_ssl_conf_t, ssl_spiffe),		
      NULL },	
    { ngx_string("ssl_spiffe_accept"),		
      NGX_STREAM_MAIN_CONF|NGX_STREAM_SRV_CONF|(NGX_CONF_TAKE1|NGX_CONF_TAKE2|NGX_CONF_TAKE3   \
                |NGX_CONF_TAKE4|NGX_CONF_TAKE5|NGX_CONF_TAKE6|NGX_CONF_TAKE7),		
      ngx_conf_set_spiffe_accept_slot,		
      NGX_STREAM_SRV_CONF_OFFSET,		
      offsetof(ngx_stream_ssl_conf_t, ssl_spiffe_accept),		
      NULL },
...
ngx_stream_ssl_handler(ngx_stream_session_t *s)
{
    ...
    if (sslcf->ssl_spiffe) {
        if (!spiffe_thread_created) {
            if (create_stream_spiffe_thread(&sslcf->ssl, 1, sslcf->verify_depth) != NGX_OK) {
                return NGX_ERROR;
            }
            spiffe_thread_created = 1;
        }
        
        while (is_stream_certificates_updated() != NGX_OK) {
            // just wait
            usleep(100000);
        }
    }
    ...
    if (sslcf->verify) {
        ...
        if (sslcf->verify == 1) {
            ...
            if (sslcf->ssl_spiffe == 1) {
                ...
                if (!ngx_is_spiffe_id_accepted(str->data, sslcf->ssl_spiffe_accept)) {
                    ngx_log_error(NGX_LOG_ERR, c->log, 0,
...
ngx_stream_ssl_create_conf(ngx_conf_t *cf)
{
    ...
    scf->ssl_spiffe = NGX_CONF_UNSET;		
    scf->ssl_spiffe_accept = NGX_CONF_UNSET_PTR;
...
ngx_stream_ssl_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    // If SPIFFE is disabled, and certs are no provided, tcp will no use certificates
    //
    if (!conf->ssl_spiffe && conf->certificates == NULL) {
        return NGX_CONF_OK;
    }

    // Certificates are no required when SPIFFE is enabled.
    //
    if (!conf->ssl_spiffe) {
    ...
    // No use provided certificates from configuration when SPIFFE is enabled.
    //
    if (!conf->ssl_spiffe && 
            ngx_ssl_certificates(cf, &conf->ssl, conf->certificates,
