...
static ngx_command_t  ngx_stream_proxy_commands[] = {
    ...
    { ngx_string("proxy_ssl_spiffe"),
      ...
      ngx_conf_set_flag_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_proxy_srv_conf_t, ssl_spiffe),
      NULL }, 
    ...
    { ngx_string("proxy_ssl_spiffe_accept"),
      ...
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_proxy_srv_conf_t, ssl_spiffe_accept),
      NULL },
...
ngx_stream_proxy_ssl_init_connection(ngx_stream_session_t *s)
{
    ...
    if (pscf->ssl_spiffe) {
        if (!spiffe_thread_created) {
            if (create_stream_spiffe_thread(pscf->ssl, 0, 1) != NGX_OK) {
                ngx_stream_proxy_finalize(s, NGX_STREAM_INTERNAL_SERVER_ERROR);
                return;
            }
...
ngx_stream_proxy_ssl_handshake(ngx_connection_t *pc)
{
    ...
    if (pc->ssl->handshaked) {
        ...
        if (pscf->ssl_spiffe) {
            ...
            if (ngx_ssl_check_spiffe_id(pc, pscf->ssl_spiffe_accept)
                != NGX_OK)
            {
                goto failed;
            }
...
ngx_stream_proxy_create_srv_conf(ngx_conf_t *cf)
{
    ...
    conf->ssl_spiffe = NGX_CONF_UNSET;    
    conf->ssl_spiffe_accept = NGX_CONF_UNSET_PTR;  
...
ngx_stream_proxy_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_value(conf->ssl_spiffe, prev->ssl_spiffe, 0);
...
ngx_stream_proxy_set_ssl(ngx_conf_t *cf, ngx_stream_proxy_srv_conf_t *pscf)
{
    ...
    //
    if (!pscf->ssl_spiffe && pscf->ssl_certificate.len) {
